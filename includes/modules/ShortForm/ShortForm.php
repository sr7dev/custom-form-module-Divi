...
class mrc_short_form_Module extends ET_Builder_Module {
	function init() {
		$this->name            = esc_html__( 'MRC Short Form', 'et_builder' );
		$this->plural          = esc_html__( 'MRC Short Forms', 'et_builder' );
		$this->slug            = 'mrc_short_form';
		$this->vb_support      = 'on';
		$this->child_slug      = 'mrc_short_field';
		$this->child_item_text = esc_html__( 'Field', 'et_builder' );
		
		
		$this->main_css_element = '%%order_class%%.mrc_short_form_container';
		...
		$this->advanced_fields = array(
			...
			'fonts' => array(
				'title' => array(
					...
					'css' => array(
						...
						{$this->main_css_element} h2.mrc_short_main_title, 
						{$this->main_css_element} h3.mrc_short_main_title, 
						{$this->main_css_element} h4.mrc_short_main_title, 
						{$this->main_css_element} h5.mrc_short_main_title, 
						{$this->main_css_element} h6.mrc_short_main_title",
					),
					'header_level' => array(
						'default' => 'h1',
					),
				...
				'default' => array(
					'css' => array(
						'main' => implode( ', ', array(
							...
							'%%order_class%% .mrc_short_field select',
							'%%order_class%% .mrc_short_field textarea',
							'%%order_class%% 
								.mrc_short_field 
								.mrc_short_field_options_list label > i',
							'%%order_class%% input.mrc_short_captcha',
						) ),
					),
				),
			),
			'button'                => array(
				'button' => array(
					...
					'box_shadow'  => array(
						'css' => array(
							'main' => '%%order_class%% .mrc_short_submit',
						),
					),
			...
			'max_width'             => array(
				'css' => array(
					'module_alignment' => 
					'%%order_class%%.mrc_short_form_container.mrc_module',
				),
			),
			'text' => array(
				'css' => array(
	...
	function render( $attrs, $content = null, $render_slug ) {
		global $mrc_half_width_counter, $mrc_short_form_num;
		...
		$mrc_short_form_num = $this->render_count();
		...
		$current_form_fields = isset( 
			$_POST['mrc_short_email_fields_' . $mrc_short_form_num] 
		) 
			? $_POST['mrc_short_email_fields_' . $mrc_short_form_num] 
			: '';
		$hidden_form_fields = isset( 
			$_POST['mrc_short_email_hidden_fields_' . $mrc_short_form_num] 
		) 
			? $_POST['mrc_short_email_hidden_fields_' . $mrc_short_form_num] 
			: false;
		$contact_email = '';
		...
		if ( $nonce_result && 
			isset( $_POST['mrc_shortform_submit_' . $mrc_short_form_num] ) && 
			empty( $_POST['mrc_shortform_validate_' . $mrc_short_form_num] ) ) {
			if ( '' !== $current_form_fields ) {
					...
					foreach( $fields_data_array as $index => $value ) {
						...
						if ( 'required' === $value['required_mark'] && 
							...
							$et_error_message .= sprintf( 
								'<p class="mrc_short_error_text">%1$s</p>', esc_html__( 
				...
				$et_error_message .= sprintf( 
					'<p class="mrc_short_error_text">%1$s</p>', 
		...
		$mrc_captcha = sprintf( '
			<div class="mrc_short_right">
				<p class="clearfix">
					<span class="mrc_short_captcha_question">%1$s</span> 
					= 
					<input 
						type="text" 
						size="2" 
						class="input mrc_short_captcha" 
						data-first_digit="%3$s" 
						data-second_digit="%4$s" 
						value="" 
						name="mrc_short_captcha_%2$s"
				...
				<div class="mrc_short">
					...
					>
						...
						<input 
							...
							name="mrc_shortform_submit_%7$s" 
						/>
						<input 
							type="text" 
							value="" 
							name="mrc_shortform_validate_%7$s"
							class="mrc_shortform_validate_field" 
							id="%12$s" 
							placeholder="Enter ZIP"
						/>
						<div class="mrc_button_container">
							%2$s
							<button 
								id="%13$s" 
								type="submit" 
								class="mrc_short_submit mrc_button%6$s"
								%5$s%9$s%10$s
							>
								%3$s
							</button>
		...
		$module_id = '' !== $this->module_id(false) 
			...
			: 'mrc_short_form_' . $mrc_short_form_num;
		...
		$output = sprintf( '
			...
			<div id="%4$s" class="%5$s" data-form_unique_num="%6$s"%7$s>
				...
				%1$s
				%3$s
			</div> <!-- .mrc_short_form_container -->
			',
			( '' !== $title 
				? sprintf( '<%2$s class="mrc_short_main_title">%1$s</%2$s>', 
			...
			$form,
			esc_attr( $module_id ),
			'mrc_short_form_container clearfix',//$this->module_classname( $render_slug ),
			esc_attr( $mrc_short_form_num ),
